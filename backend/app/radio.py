...
async function updateLocation(){try{const p=await getPosition();badge('location-permission','Granted','ok');if(state.seq===0){await api('/api/v1/me/location-consent',{method:'PUT',body:JSON.stringify({policy_version:state.config.location_policy_version,disclosure_version:state.config.location_disclosure_version})});}state.seq++;await api('/api/v1/me/location',{method:'PUT',body:JSON.stringify({observed_at:new Date(p.timestamp).toISOString(),latitude:p.coords.latitude,longitude:p.coords.longitude,horizontal_accuracy_m:p.coords.accuracy,heading_deg:Number.isFinite(p.coords.heading)?p.coords.heading:null,speed_mps:Number.isFinite(p.coords.speed)?Math.max(0,p.coords.speed):null,client_sequence:state.seq,consent_policy_version:state.config.location_policy_version})});}catch(e){badge('location-permission','Blocked','bad');throw new Error('Location access is required so RoadTalk can determine nearby listeners.');}}
