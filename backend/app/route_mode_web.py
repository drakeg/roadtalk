...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:Inter,system-ui,sans-serif;min-height:100vh}.wrap{width:min(760px,calc(100% - 28px));margin:auto}header{border-bottom:1px solid var(--line);padding:18px 0}.nav{display:flex;align-items:center;justify-content:space-between;gap:16px}.brand{font-weight:850}a{color:var(--text)}main{padding:38px 0}.card{border:1px solid var(--line);background:var(--panel);border-radius:18px;padding:22px;margin-bottom:16px}.eyebrow{color:var(--accent);font-size:11px;font-weight:900;letter-spacing:.16em;text-transform:uppercase}h1{font-size:clamp(30px,6vw,46px);margin:8px 0 12px}.body{color:var(--muted);line-height:1.6}.options{display:grid;gap:12px}.option{width:100%;text-align:left;border:2px solid var(--line);background:rgba(255,255,255,.025);color:var(--text);padding:18px;border-radius:14px;cursor:pointer}.option[aria-pressed="true"]{border-color:var(--accent)}.option:disabled{opacity:.55;cursor:not-allowed}.option strong{display:block;font-size:19px;margin-bottom:6px}.status{font-weight:750}.good{color:var(--green)}.bad{color:var(--red)}
  </style>
</head>
<body>
<header><div class="wrap nav"><div class="brand">RoadTalk · Audience Mode</div><a href="/">Back to Web Radio</a></div></header>
<main class="wrap">
