...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .section-label { color:var(--muted); font-size:12px; font-weight:800; letter-spacing:.08em; text-transform:uppercase; margin:22px 0 10px; }
    ...
    .activity { display:grid; grid-template-columns:repeat(5,1fr); gap:14px; margin-bottom:14px; }
    ...
    @media(max-width:1050px){ .activity{grid-template-columns:repeat(3,1fr)} }
    @media(max-width:900px){ .metrics{grid-template-columns:repeat(2,1fr)} .grid-main,.lower{grid-template-columns:1fr} }
    @media(max-width:600px){ .metrics,.activity{grid-template-columns:1fr} nav,.title-row{align-items:flex-start;flex-direction:column} .log-entry{grid-template-columns:66px 48px 1fr} }
  </style>
</head>
<body>
<header><div class="wrap"><nav><div class="brand"><div class="mark">RT</div><div>RoadTalk <span style="color:var(--muted);font-weight:600">Local Ops</span></div></div><div class="navlinks"><a class="button" href="/docs">Swagger</a><a class="button" href="/redoc">ReDoc</a><a class="button" href="/health/ready">Raw readiness</a><a class="button" href="/api/v1/system/metrics">Metrics JSON</a><a class="button" href="/api/v1/system/version">Version JSON</a></div></nav></div></header>
<main class="wrap">
  <section class="title-row row"><div><div class="eyebrow">Local operations dashboard</div><h1>RoadTalk service overview</h1><p class="subtitle">Live local health, platform activity, latency, environment, and browser-observed operational events.</p></div><div class="status-pill"><span id="status-dot" class="dot"></span><span id="status-text">Checking services…</span></div></section>
  ...
  <section class="metrics">
    ...
    <article class="card metric"><div class="metric-label">Latest latency</div><div class="metric-value"><span id="latency-value">—</span><span style="font-size:14px;color:var(--muted)"> ms</span></div><div class="metric-foot">Health + metrics round trip</div></article>
  ...
  <section class="activity">
    <article class="card metric"><div class="metric-label">Active accounts</div><div class="metric-value" id="accounts-value">—</div><div class="metric-foot">Non-disabled, non-deleted accounts</div></article>
    <article class="card metric"><div class="metric-label">Active locations</div><div class="metric-value" id="locations-value">—</div><div class="metric-foot">Unexpired current locations</div></article>
    <article class="card metric"><div class="metric-label">Enabled channels</div><div class="metric-value" id="channels-value">—</div><div class="metric-foot">Public and private channels</div></article>
    <article class="card metric"><div class="metric-label">Channel memberships</div><div class="metric-value" id="memberships-value">—</div><div class="metric-foot">Active memberships</div></article>
    <article class="card metric"><div class="metric-label">Valid media grants</div><div class="metric-value" id="grants-value">—</div><div class="metric-foot">Unexpired, non-revoked grants</div></article>
  </section>
  ...
  <section class="grid-main">
    ...
    <article class="card panel"><div class="card-head"><h2>Service health</h2><span class="tag">live</span></div><div class="service-list">
      ...
      <div class="service"><div class="row"><div><div class="service-name">Operational metrics</div><div class="service-detail">/api/v1/system/metrics</div></div><span id="metrics-dot" class="mini-dot"></span></div></div>
...
<footer><div class="wrap">RoadTalk local alpha · Aggregate activity only; no callsigns, coordinates, invite secrets, or participant identifiers are exposed here.</div></footer>
<script>
  ...
  async function poll(){const started=performance.now(); try{const [liveResp,readyResp,versionResp,metricsResp]=await Promise.all([fetch('/health/live',{cache:'no-store'}),fetch('/health/ready',{cache:'no-store'}),fetch('/api/v1/system/version',{cache:'no-store'}),fetch('/api/v1/system/metrics',{cache:'no-store'})]); const elapsed=Math.round(performance.now()-started); if(!liveResp.ok||!readyResp.ok||!versionResp.ok||!metricsResp.ok)throw new Error('One or more RoadTalk endpoints returned an error'); const live=await liveResp.json();const ready=await readyResp.json();const version=await versionResp.json();const metrics=await metricsResp.json(); const healthy=live.status==='ok'&&ready.status==='ready'; successPolls++; samples.push({ms:elapsed}); if(samples.length>30)samples.shift(); $('latency-value').textContent=elapsed;$('poll-value').textContent=successPolls;$('ready-value').textContent=healthy?'READY':'DEGRADED';$('ready-foot').textContent=healthy?'All registered checks passed':'One or more checks failed'; $('accounts-value').textContent=metrics.active_accounts;$('locations-value').textContent=metrics.active_locations;$('channels-value').textContent=metrics.enabled_channels;$('memberships-value').textContent=metrics.active_memberships;$('grants-value').textContent=metrics.valid_media_grants; $('environment').textContent=version.environment;$('version').textContent=version.version;$('last-refresh').textContent=timestamp(); const db=ready.checks&&Object.prototype.hasOwnProperty.call(ready.checks,'database')?ready.checks.database:'not registered'; $('database-status').textContent=db; setDot('api-dot',live.status==='ok');setDot('version-dot',true);setDot('metrics-dot',true);setDot('db-dot',db==='ready'||db==='not registered'); $('status-dot').className='dot '+(healthy?'ok':'bad');$('status-text').textContent=healthy?'RoadTalk ready':'RoadTalk degraded'; if(lastState!==String(healthy)){log(healthy?'OK':'ERROR',healthy?'RoadTalk readiness is healthy.':'RoadTalk readiness is degraded.');lastState=String(healthy);} else {log('INFO',`Health poll completed in ${elapsed} ms; accounts=${metrics.active_accounts}, locations=${metrics.active_locations}, channels=${metrics.enabled_channels}.`);} drawChart(); }catch(err){samples.push({ms:Math.round(performance.now()-started)});if(samples.length>30)samples.shift();$('ready-value').textContent='DOWN';$('ready-foot').textContent='Health poll failed';$('status-dot').className='dot bad';$('status-text').textContent='RoadTalk unavailable';setDot('api-dot',false);setDot('db-dot',false);setDot('metrics-dot',false);setDot('version-dot',false);log('ERROR',err.message||'Health poll failed.');drawChart();}}
